...
const showTabBar = computed(() => {
  const mainPaths = ['/mobile/notes', '/mobile/chat', '/mobile/setting']
...
<template>
  <div class="flex flex-col h-screen overflow-hidden bg-slate-50 dark:bg-slate-900">
    <!-- 主内容区域 - 渲染子路由 -->
    <main class="flex-1 overflow-y-auto" :class="showTabBar ? 'pb-[72px]' : ''">
    ...
    <transition name="slide-up">
      <footer 
        v-show="showTabBar"
        class="fixed bottom-0 left-0 right-0 h-[72px] bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center px-6 pb-2 pt-1 shadow-[0_-4px_10px_rgba(0,0,0,0.03)] z-50 rounded-t-3xl"
      >
